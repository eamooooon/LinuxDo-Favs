...
- 收藏帖子或回复时会保存一行内容摘要，旧收藏在当前页展示时也会按需补全摘要
- 设置页新增“内容预览”，支持不显示、显示全部、仅显示回复三种模式
- 收藏面板会低频检测内容状态：仅检查当前页、每批最多 8 条并缓存 7 天，遇到 `404` 或 `410` 时标记“内容已删除”
- 优化内容预览与状态检测的页面更新方式，只原位更新对应条目，修复切换文件夹和批量删除后列表连续闪烁两次的问题
- 修复只收藏主贴时同一主题下所有回复都错误显示为已收藏的问题，主贴和回复现在按各自帖子 ID 精确匹配
- 优化批量操作栏的固定位置，并将删除确认框放在删除按钮下方且按右边缘对齐
- 优化拖拽移动样式，拖动条目和目标收藏夹使用当前主题色；虚线框改为向内绘制，避免边框外扩引起布局跳动
- 重新整理设置页选项布局：显示模式与内容预览使用一致的三等分尺寸、左右栏垂直对齐，文字颜色与其他选项统一
- 主题颜色改为单行自适应排列，并按红、橙、黄、绿、青、蓝、紫、粉到中性色的连续色阶排序
...
- **内容预览**：收藏条目可显示一行帖子或回复摘要，旧收藏会在浏览时按需补全
- **内容状态**：低频检测当前页收藏是否已被删除，并显示“内容已删除”提示
